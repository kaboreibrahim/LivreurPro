...
class DCL(SafeDeleteModel, LifecycleModel):
    ...
    def get_roadmap(self):
        ...
        # Liste des étapes avec leur statut, description et icône
        etapes = [
            {
                'id': 'EN_ATTENTE',
                'title': 'En attente',
                'description': 'En attente de proposition de prix',
                'icon': 'bi-hourglass-split',
                'status': 'pending',
                'details': 'Vous allez recevoir un appel sous peu.'
            },
            {
                'id': 'VALIDATION_CLIENT',
                'title': 'Validation client',
                'description': 'Validation du prix',
                'icon': 'bi-person-check',
                'status': 'pending',
                'details': 'Veuillez valider le prix proposé.'
            },
            {
                'id': 'VALIDATION_LIVREUR',
                'title': 'Validation livreur',
                'description': 'En attente du livreur',
                'icon': 'bi-person-workspace',
                'status': 'pending',
                'details': 'Recherche d\'un livreur disponible.'
            },
            {
                'id': 'LIVREUR_ROUTE',
                'title': 'Livreur en route',
                'description': 'Vers le point de ramassage',
                'icon': 'bi-truck',
                'status': 'pending',
                'details': 'Le livreur est en chemin pour récupérer votre colis.'
            },
            {
                'id': 'RECEPTION_COLIS',
                ...
                'icon': 'bi-box-seam',
                'status': 'pending',
                'details': 'Votre colis a été pris en charge.'
            },
            {
                'id': 'LIVRAISON_EN_ROUTE',
                'title': 'En livraison',
                'description': 'Vers la destination',
                'icon': 'bi-truck',
                'status': 'pending',
                'details': 'Votre colis est en cours de livraison.'
            },
            {
                'id': 'TERMINEE',
                ...
                'status': 'pending',
                'details': 'Votre colis a été livré avec succès.'
            },
            {
                'id': 'ANNULEE',
                ...
                'icon': 'bi-x-circle',
                'status': 'cancelled',
                'details': 'Cette commande a été annulée.'
            }
        ]

        # Trouver l'index de l'étape actuelle
        statut_index = next((index for index, etape in enumerate(etapes) if etape['id'] == self.statut), -1)
        
        # Mettre à jour le statut des étapes
        for index, etape in enumerate(etapes):
            if index < statut_index:
                etape['status'] = 'completed'
            elif index == statut_index:
                etape['status'] = 'active'
            else:
                etape['status'] = 'pending'
        
        # Si le statut est annulé, marquer toutes les étapes comme annulées
        if self.statut == 'ANNULEE':
            for etape in etapes:
                etape['status'] = 'cancelled'
        
        # Générer le HTML de la timeline
        html = '<div class="timeline-container">'
        
        for etape in etapes:
            # Sauter l'étape annulée si la commande n'est pas annulée
            if etape['id'] == 'ANNULEE' and self.statut != 'ANNULEE':
                continue
                
            html += f"""
            <div class="timeline-step {etape['status']}">
                <div class="timeline-marker">
                    <i class="bi {etape['icon']}"></i>
                </div>
                <div class="timeline-content">
                    <div class="timeline-header">
                        <h5 class="timeline-title">{etape['title']}</h5>
                        <span class="timeline-badge">{etape['description']}</span>
                    </div>
                    <p class="timeline-details">{etape['details']}</p>
                </div>
            </div>
            """
